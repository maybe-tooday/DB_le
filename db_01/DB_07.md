...
```python
dic [*Director.objects.all().values('name','debut','country')]
...
```python
dir_ = Director.objects.get(id=1)
print(dir_.name, dir_.debut, dir_.country)
...
> 이유 작성
출력해줄 값이 아무것도 없기 떄문에 오류가 뜬거 같습니다.
아래의 경우 출력해줄 값이 여러개 여서 오유가 뜬거 같습니다.
> 
